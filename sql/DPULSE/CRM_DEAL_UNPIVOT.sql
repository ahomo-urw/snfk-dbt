create
or replace view DPULSE.CRM_DEAL_UNPIVOT COMMENT = 'AGAUDET: Created for external vizualisation tool and Plan-It Interface- modif AHOMO202505' as with lt_deal_referential as (
    select
        *
    from
        dpulse.crm_deal_w_key
),
--table temporaire crm_deal_w_key
...
lt_rent_incentives as (
    select
        ...
        decode(
            ...
            'RENT_FREE_PERIOD',
            'ERROR_RI'
        ) as indicator_name --cap_typecode --typecode
,
...
,
        ...
        dl.ESTIMATEDSIGNATUREDATE,
        dl.SIGNATURE_DATE_RPT
    from
        dpulse.crm_steprent as ri
...
lt_deal_pivot as (
    select
        ...
        indicator_amount,
        case
            when lower(indicator_name) = 'available_indicator_deal' then ''
            else currency
        end as currency,
        estimated_handover_date as startdate,
        ifnull(enddate, estimated_handover_date) as enddate --aho 2025
,
        DEAL_DESC,
        ...
        SIGNATURE_DATE_RPT
    from
        lt_deal_referential unpivot (
            indicator_amount for indicator_name --liste des indicateurs recupérés
            in (
                available_indicator_deal,
                New_Retail_MGR,
                mgr_performance,
                economic_rent,
                effective_rent,
                ...
                expectedsales,
                ECORENTPERFPERC,
                ECORENTPERFPERCLFL --,ECONOMICRENTM
,
                ECONOMICRENTPERFORMANCE,
                EFFRENTPERC,
                EFFRENTPERCLFL --,EFFECTIVERENTM
,
                ...
                MGRLFL,
                MGRPERFLFL,
                EFFECTIVERENTLFL --,EFFECTIVERENTM2LFL
,
                EFFECTIVERENTPERFLFL,
                ECONOMICRENTLFL --,ECONOMICRENTM2LFL
,
                ECONOMICRENTPERFLFL,
                YEARLY_SBR_RDL,
                BCOM_RDL --------------------------------------------------ASECK NRI 04022025
,
                qu_nriimpactn,
                qu_nriimpactnplus1,
                qu_nrivslon,
                qu_nrivslonplus1,
                qu_nrivsoldn,
                qu_nrivsoldnplus1 --,qu_oldnrilfln
                --,qu_oldnrilflnplus1
            )
        )
),
...
lt_deal_pivot_Dates as (
    ...
    from
        lt_deal_referential unpivot (
            startdate for indicator_name in (
                Signature_Date_RPT,
...
select
    ...
    quote_id,
    indicator_name
    /*, 1        as indicator_amount aho20250906*/,
    cast(TO_CHAR(startdate, 'YYYYMMDD') as number) as indicator_amount,
    '' as currency,
...
,
    ...
    null as ESTIMATEDSIGNATUREDATE,
    null as SIGNATURE_DATE_RPT
from
    lt_deal_pivot_Dates