...
select
  /*  d.created_on as DEAL_CREATION,
    ...
    f.*
from
--DPULSE.t_fact_lo_deal f
--join 
--dpulse.t_dim_deal d on d.deal_id=f.deal_id
...
select
    *
from
    DPULSE.CRM_RENTALUNIT_W_KEY--T_CRM_RENTALUNIT_W_KEY --attention penser a rafraichir la table !!!!
),
...
from
  ...
  -- where
   --  fact.RU_USAGE_CALC <> 'None' -- EVOL SGE pour récuprer les dates
    --and fact.deal_id='dc477580-d5c5-ee11-9079-000d3ab4bcd9'
 --   and 1=0
 --) select count(*) from  lt_deal_lo_fact where indicator_name like '%DATE%';
    union all
    select
    ru.PC_ID,
    ru.buextref,
   null /* dqr.YM_START || '-' || dqr.YM_END as*/ YM_START_END,
   null /* dqr.YM_START as */  YM_START,
   null /* dqr.YM_END as*/  YM_END,
    fact.DEAL_CREATION,
    fact.DEAL_FINANCIALPROPOSALAGREED,
    fact.DEAL_DATEVALIDATED,
    fact.DEAL_SIGNATUREDATE,
...
from
    lt_deal_and_lo_pivot as fact
  --  JOIN lt_crm_deal_quote_relevant dqr ON fact.quote_id = dqr.QUOTEID  or  -- on recup les LO sans DEAL --on vire les not relevant par inner join
    LEFT JOIN lt_crm_leasingorder lo ON lo.leasing_order_id = fact.leasing_order_id
    left join lt_rental_unit as ru on ru.leasing_order_id = fact.leasing_order_id
...
--select distinct Object_Type from lt_final_currency;--4,8M
...
, lt_lo_wo_deal as (
    select
       ...
       --Deal.CREATEDON,
        ...
        else null end LO_START_YM,
        min(to_char(Deal.CREATEDON,'YYYYMMDD')) LO_END_YM,
    from
        lt_crm_leasingorder lo
        left join LT_CRM_DEAL Deal on deal.leasing_order_id=lo.leasing_order_id 
    ...
    where
       ...
       -- and 
        ...
        and (lo.PLANITOPERATIONCODE like 'BP%' or lo.PLANITOPERATIONCODE like 'FQ%')
        group by 
        lo.leasing_order_id ,
        lo.id ,
        lo.planitoperationcode,
        --Deal.CREATEDON,
        LO_START_YM
        having ifnull(LO_END_YM,'999912')>LO_START_YM
)
--select * from lt_fact_w_main_lo ;
...
, lt_fact_deal as (
    select -- DEAL PRODUCT 
        ...
        null /*mn.LEASING_ORDER_ID */ AS LO_GUID,
        null /*mn.LO_ID */ AS LO_ID,
    ...
    from
      ...
      where mn.Object_Type='DL' -- que les dEAL
      and--  (mn.DEAL_GUID is not null or 
      length(mn.DEAL_GUID)>1
      --lo.LO_GUID is null
     --   left join lt_lo_wo_deal lo on mn.leasing_order_id = lo.LO_GUID
     UNION ALL -- ajoutet des LO au format deal a 0%
     select
        --2 AS COL,
        --null MAIN_LEASING_ORDER,
        mn.PC_ID,
        ...
        -- mn.DEAL_LANDLORDSIGNATUREDATE,
        null /*mn.LEASING_ORDER_ID*/ AS LO_GUID,
        null /*lo.LO_ID */ AS LO_ID,
        lo.planitoperationcode as DEAL_ID,
        mn.LEASING_ORDER_ID  DEAL_GUID,
        mn.BRAND_GUID,
        mn.BRAND_ID,
        mn.LESSEE_GUID,
        mn.LESSEE_ID,
    ...
    from
       ...
       where mn.Object_Type='LO' -- que les dEAL
        
    union all
       select -- LISTE les leasing order product
       -- null MAIN_LEASING_ORDER,
       --3 AS COL,
        mn.PC_ID,
        mn.buextref,
        ...
        else to_char(CURRENT_DATE,'YYYY')||'0101-99991231' end
       YM_START_END,
         case 
              when lo.planitoperationcode like 'BP%' then  right(left(lo.planitoperationcode,6),4)||'0101'
              when lo.planitoperationcode like 'FQ1%' then right(left(lo.planitoperationcode,7),4)||'0101'
              when lo.planitoperationcode like 'FQ2%' then right(left(lo.planitoperationcode,7),4)||'0401'
              when lo.planitoperationcode like 'FQ3%' then right(left(lo.planitoperationcode,7),4)||'0701' 
        else to_char(CURRENT_DATE,'YYYY')||'0101' end YM_START,
                case 
    ...
    from
       ...
       where mn.Object_Type='LO' -- tout les LO
        
    union all
       -- SGE202506: vacancy planit du rent roll pour reporting *PBI      
    SELECT -- LISTE les contrat product Planit 
    ...
    SELECT -- LISTE les product Planit not finished
       -- NULL AS MAIN_LEASING_ORDER,
       --5 AS COL,
        PC_ID,
        bu_extref,
...
, lt_full_w_expvac as (
    select
        ...
        case when tf.DEAL_SIGNATUREDATE is null then tf.deal_SIGNATURE_DATE_RPT else tf.DEAL_SIGNATUREDATE end DEAL_SIGNATUREDATE, 
        case when tf.DEAL_SIGNATUREDATE is not null then 'Reel'   else 'Estimated'   end as SIGNATUREDATE_TYPE,
        ...
        tf.CONTRACT_ID,
        max(case 
            when s.status = 'Occupied' and left(s.TERMDATE,4)=left(s.ym_end,4) then 'EXPIRY' -- tu es expiry si c'est sur la meme année
        ...
        max(case 
            when s.status = 'Occupied' then 
            left(s.TERMDATE, 6)
            else left(s.occupancystartdate, 6) end
...
,lt_percentage as (
select
    distinct percentage,deal_guid--,ym_start
from    
    lt_full_w_expvac),
...
from
    lt_percentage s 
    --join lt_dim_calendar t on t.cal_id between s.ym_start and s.ym_end 
group by deal_guid--,ym_start
)

, lt_flag_active as (select 
    case when a.maxpct is not null then 'Active'
    else null
    end as Active
    ,fact.*
from 
    lt_full_w_expvac fact left join lt_active a
    on a.deal_guid=fact.deal_guid
    and a.maxpct=fact.percentage)
    
select * from lt_flag_active;
