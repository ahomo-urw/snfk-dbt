select *
from 
EXPOSE.DPULSE.CRM_DEAL_QUOTE_RELEVANT
where 
ID='Deal-00064579'
--opportunityid='62df2299-5bd7-ef11-a72f-000d3a6869af';
 ;

 select signaturedate from finops.dwh.crm_deal 
where opportunityid='62df2299-5bd7-ef11-a72f-000d3a6869af'; 

create or replace view DPULSE.CRM_DEAL_QUOTE_RELEVANT(
	OPPORTUNITYID,
	ID,
	QUOTEID,
	FACT_PROG,
	...
	YM_START,
	YM_END
) COMMENT='SGE 202506: more sifgnificant view for each step of the deal for PBI'
 as
WITH LT_10 AS (
	SELECT 
		d.opportunityid, 
...
, LT_70 AS (
	SELECT 
		...
		CASE WHEN max(q.createdon) OVER (PARTITION BY d.opportunityid) = q.createdon 
			...
			ELSE false 
		END filter,
		TO_CHAR(firt_val_quote,'YYYYMMDD') YMD_START,
		NVL(TO_CHAR(datelegalagreement,'YYYYMMDD'),'99991231') YMD_END
		/*,progressiontech, datefinancialproposalagreed, datevalidated, datelegalagreement, d.createdon deal_createon, landlordsignaturedate, signaturedate,*/
	FROM finops.dwh.crm_deal d 
	LEFT JOIN finops.dwh.crm_quote q ON q.opportunityid = d.opportunityid
    left join (select cap_quoteid, max(cap_responsedate)  firt_val_quote
                from finops.dwh.t_cap_approval
                group by cap_quoteid) val on val.cap_quoteid=q.quoteid
	WHERE --d.opportunityid='f1b05667-7d41-4bbc-9925-58e99d03ec9c'--'5b6b7053-7c73-4c7c-bd57-3fbf65bf46e9' AND
